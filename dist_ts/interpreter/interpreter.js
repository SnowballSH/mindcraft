...
function bubbleFromChild(state, parentId, childStatus) {
...
export async function tick(state, registry, injectedCtx = {}) {
    const ctx = {
        state,
    ...
    if (state.runtime.running) {
        ...
        if (res.updates)
            Object.assign(state, res.updates);
    ...
    while (true) {
        ...
        if (node.type === 'Action') {
            ...
            if (res.status === 'RUNNING') {
                running(state, node.id, {
                    ...
                    params: resolveVarsInRecord((node.params ?? {}), state.vars),
        ...
        throw new Error(`Unsupported node type in interpreter: ${node.type}`);
    }
}
