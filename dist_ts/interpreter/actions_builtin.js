...
export function createDefaultRegistry() {
    ...
    registerActionWithAliases(r, 'goto_block_from_bb', z.object({
        ...
        requireAgent(ctx);
        const caps = requireCapabilities(ctx);
        const list = (ctx.state.blackboard ?? {})[args.from];
    ...
    registerActionWithAliases(r, 'break_block_from_bb', z.object({
        from: z.string().default('nearbyBlocks'),
        index: z.coerce.number().int().min(0).default(0),
    }), async (ctx, args) => {
        requireAgent(ctx);
