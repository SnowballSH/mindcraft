...
export function registerDefaultConditions(r) {
    ...
    r.registerCondition({
        ...
        async evaluate(ctx, args) {
            const v = (ctx.state.blackboard ?? {})[args.key];
    ...
    r.registerCondition({
        ...
        async evaluate(ctx, args) {
            ...
            const ok = v.some((item) => String(item?.[args.field] ?? '') === args.value);
            return { status: ok ? 'SUCCESS' : 'FAILURE' };
        },
    });
    r.registerCondition({
        ...
        async evaluate(ctx, args) {
            ...
            if (!Array.isArray(v))
                return { status: 'FAILURE' };
            const ok = v.some((item) => Number(item?.[args.field]) <= args.max);
            return { status: ok ? 'SUCCESS' : 'FAILURE' };
        },
    });
    r.registerCondition({
    ...
    r.registerCondition({
        ...
        async evaluate(ctx, args) {
            ...
            const bbInv = (ctx.state.blackboard ?? {})[args.inventory_key];
            if (bbInv && typeof bbInv === 'object') {
                const haveFromBb = Number(bbInv[args.item] ?? 0);
