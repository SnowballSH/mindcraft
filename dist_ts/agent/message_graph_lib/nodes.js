...
export function makeMessageGraphNodes(agent, deps) {
    function buildPersonaPreamble() {
        ...
        const raw = String(agent?.prompter?.profile?.conversing ?? '');
        if (!raw)
            return '';
    ...
    async function forcedUserCommand(state) {
        ...
        const result = await deps.commands.executeCommand(agent, state.messageEn);
        if (result)
            await agent.routeResponse(state.source, String(result));
        return { done: true, usedCommand: true };
    }
    async function injectBehaviorLog(_state) {
    ...
    async function complexityClassify(state) {
        ...
        try {
            const out = await classifyComplexity(state.messageEn, {
                ...
                getCommandDocs: () => deps.commands.getCommandDocs(agent),
                personaPreamble,
            });
    ...
    async function runPlanExecute(state) {
        ...
        if (!lcModel) {
            ...
            await agent.routeResponse(state.source, `plan_execute is disabled because no LangChain model was constructed for profile model '${profileModel}'. ` +
                `Configure provider API keys / model so structured output is available.`);
            return { done: true, usedCommand: false };
        }
        await runPlanAndExecuteGraph(agent, {
            lcModel,
            personaPreamble,
            getCommandDocs: () => deps.commands.getCommandDocs(agent),
            ...
            btRecursionLimit: deps.settings?.bt_recursion_limit ?? undefined,
            btMaxRepairs: deps.settings?.bt_max_repairs ?? undefined,
            btMaxNodes: deps.settings?.bt_max_nodes ?? undefined,
        }, state.source, state.messageEn, { recursionLimit: deps.settings.plan_execute_recursion_limit ?? 50 });
        return { done: true, usedCommand: true };
    }
    async function runDirectExecute(state) {
        ...
        try {
            ...
            if (decision.decision === 'direct_command' && decision.command) {
                ...
                const result = await deps.commands.executeCommand(agent, cmdMsg);
                if (result) {
        ...
        const execRes = await deps.commands.executeCommand(agent, truncated);
        if (execRes)
            await agent.history.add('system', String(execRes));
        agent.history.save();
        if (execRes)
            await agent.routeResponse(state.source, String(execRes));
        return { done: true, usedCommand: true };
    }
    function routeAfterForced(state) {
