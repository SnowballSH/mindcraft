...
function registerActionWithAliases<TArgs>(
  r: Registry,
  name: string,
  ...
  if (!r.hasAction(name)) r.registerAction({ name, schema, run });
  if (!r.hasAction(snakeName)) r.registerAction({ name: snakeName, schema, run });
}
