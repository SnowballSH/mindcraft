...
export function buildMessageGraph(agent: AgentAdapter, depsOverride?: Partial<MessageGraphDeps>) {
  const deps: MessageGraphDeps = { ...defaultDeps, ...(depsOverride ?? {}) } as MessageGraphDeps;
  deps.commands = { ...defaultDeps.commands, ...((depsOverride?.commands ?? {}) as Partial<MessageGraphDeps['commands']>) };
